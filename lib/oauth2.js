...
function Strategy(options, verify) {
  ...
  options.scope = options.scope || {'r_basicprofile': ['id', 'first-name', 'last-name', 'picture-url', 'formatted-name']};
...
Strategy.prototype._convertScopeToUserProfileFields = function(scope) {
  ...
  }else{ // User can pass in scope as object with the
         // appropriate fields for that scope. 
    
    if(!scope["r_basicprofile"] && scope["r_fullprofile"]){
        Array.prototype.push.apply(fields, _.intersection(scope['r_fullprofile'], map['r_basicprofile']));
    }

    for(var perm in scope){
      if(scope[perm]){
        Array.prototype.push.apply(fields, _.intersection(scope[perm], map[perm]));
      }
    }
  }
